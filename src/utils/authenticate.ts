...
export const authorize = async (
  params: AuthorizationQueryParameter
): Promise<RedirectQueryParameter> => {
  return browser.identity
    .launchWebAuthFlow({
      url:
        "https://accounts.spotify.com/authorize?" +
        new URLSearchParams(
          params as Required<AuthorizationQueryParameter>
        ).toString(),
      interactive: true,
    })
    .then((responseUrl) => {
...
export const exchangeForToken = async (
  params: TokenRequestBody | RefleshedTokenRequestBody
...
export const authenticate = async (
  ...
  if (state !== responseState) {
    return new UnmatchStateError(
      "The `state` value of the response is not match the stored `state` value."
    );
  }
